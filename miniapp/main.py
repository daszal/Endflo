...
@app.get("/api/triage/{triage_id}/messages")
def api_get_triage_messages(triage_id: int, request: Request):
    """Source messages a triage item was extracted from (for the detail popup)."""
    schema = _get_client_schema(request)
    msgs = db.get_triage_messages(schema, triage_id)
    if msgs is None:
        raise HTTPException(404, "Triage item not found")
    return {"messages": [
        {
