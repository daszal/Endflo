...
(function () {
  ...
  /* ============================================================
     Real product elements — LOOPING entry animations.
     Each element assembles (frames rise, chat bubbles type in,
     fresh table rows pulse), holds long enough to read, fades,
     and repeats — but only while it's on screen. Deliberately
     slow so the "message → work done" story is legible.
     Everything stays fully visible without JS / reduced motion.
     ============================================================ */

  var TERRA = 'rgba(194,91,61,';   /* flash colour for new rows */

  function realChildren(el) {
    return Array.prototype.filter.call(el.children, function (c) {
      return !c.classList.contains('conn');
    });
  }

  /* build one looping assemble → hold → fade timeline, initially paused */
  function buildRealLoop(el) {
    var pieces  = realChildren(el);
    var bubbles = el.querySelectorAll('.bubble');
    var newRows = el.querySelectorAll('.sheet .new, .dtable .new');
    var conn    = el.querySelectorAll('.conn path');

    var tl = gsap.timeline({ repeat: -1, repeatDelay: 1.2, paused: true });

    /* hidden starting state (rendered immediately, so nothing flashes in early) */
    tl.set(pieces, { autoAlpha: 0, y: 34 }, 0);
    if (bubbles.length) tl.set(bubbles, { autoAlpha: 0, scale: 0.72, transformOrigin: '20% 90%' }, 0);
    if (conn.length)    tl.set(conn, { autoAlpha: 0 }, 0);

    /* frames / windows / cards rise in */
    tl.to(pieces, { autoAlpha: 1, y: 0, duration: 0.8, stagger: 0.26, ease: 'power2.out' }, 0.3);

    /* chat bubbles type in, one at a slow readable beat */
    if (bubbles.length) tl.to(bubbles, { autoAlpha: 1, scale: 1, duration: 0.55, stagger: 0.62, ease: 'back.out(1.6)' }, '>-0.15');

    /* connectors fade on */
    if (conn.length) tl.to(conn, { autoAlpha: 1, duration: 0.7 }, '<0.1');

    /* freshly-written rows pulse terracotta a few times */
    newRows.forEach(function (r) {
      tl.fromTo(r,
        { backgroundColor: TERRA + '0)' },
        { backgroundColor: TERRA + '0.4)', duration: 0.5, yoyo: true, repeat: 3, ease: 'power1.inOut' },
        '>-0.1');
    });

    /* hold the finished result long enough to read, then fade for the next pass */
    var HOLD = '+=3';
    tl.to(pieces, { autoAlpha: 0, duration: 0.7, ease: 'power1.in' }, HOLD);
    if (bubbles.length) tl.to(bubbles, { autoAlpha: 0, duration: 0.5 }, '<');
    if (conn.length)    tl.to(conn, { autoAlpha: 0, duration: 0.5 }, '<');

    return tl;
  }

  /* play the loop only while the element is on screen, and only start
     once it's genuinely in view — when its centre reaches the viewport
     bottom, i.e. half the element is visible */
  function loopReal(el, tl) {
    ScrollTrigger.create({
      trigger: el,
      start: 'center bottom',
      end: 'center top',
  ...
  gsap.utils.toArray('[data-real]').forEach(function (el) {
    loopReal(el, buildRealLoop(el));
  });

  /* hero: waits until the phone is half in view, then it rises in once and
     the conversation replays on a loop (same "center bottom" gate as the rest) */
  var heroStage = document.querySelector('[data-hero-stage]');
  if (heroStage) {
    var hDevice  = heroStage.querySelector('.device');
    var hBubbles = heroStage.querySelectorAll('.bubble');
    var hCards   = heroStage.querySelectorAll('.rc');
    var hConn    = heroStage.querySelectorAll('.conn path');

    /* hidden until the phone is scrolled into view */
    if (hDevice) gsap.set(hDevice, { autoAlpha: 0, y: 46 });
    var deviceIn = hDevice
      ? gsap.to(hDevice, { autoAlpha: 1, y: 0, duration: 0.9, ease: 'power3.out', paused: true })
      : null;

    var htl = gsap.timeline({ repeat: -1, repeatDelay: 1.4, paused: true });
    htl.set(hBubbles, { autoAlpha: 0, scale: 0.72, transformOrigin: '20% 90%' }, 0)
       .set(hCards, { autoAlpha: 0, y: 22 }, 0)
       .set(hConn, { autoAlpha: 0 }, 0)
       /* let the phone settle first, then type the conversation */
       .to(hBubbles, { autoAlpha: 1, scale: 1, duration: 0.55, stagger: 0.66, ease: 'back.out(1.6)' }, 0.8)
       .to(hConn, { autoAlpha: 1, duration: 0.7 }, '<0.4')
       .to(hCards, { autoAlpha: 1, y: 0, duration: 0.65, stagger: 0.35, ease: 'power2.out' }, '<')
       .to([].slice.call(hBubbles).concat([].slice.call(hCards)), { autoAlpha: 0, duration: 0.7 }, '+=3.4')
       .to(hConn, { autoAlpha: 0, duration: 0.5 }, '<');

    var startHero = function () { if (deviceIn) deviceIn.play(); htl.play(); };
    ScrollTrigger.create({
      trigger: heroStage,
      start: 'center bottom',
      end: 'center top',
      onEnter: startHero,
      onEnterBack: startHero,
      onLeave: function () { htl.pause(); },
      onLeaveBack: function () { htl.pause(); }
    });

    /* if the phone is already at least half in view on load, kick it off */
    var hr = heroStage.getBoundingClientRect();
    var hCenter = hr.top + hr.height / 2;
    if (hCenter > 0 && hCenter < window.innerHeight) startHero();
  }
